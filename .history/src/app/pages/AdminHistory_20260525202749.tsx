...
import { History, Search, Settings, User as UserIcon, Shield } from 'lucide-react';
...
export default function AdminHistory() {
  ...
  const filteredLogs = logs
    .filter(log => log.category === 'system') // Only show system logs
    .filter(log => {
      const matchesSearch = 
  ...
  const getActivityIcon = (action: string) => {
    // Determine if action is about user or admin
    if (action.toLowerCase().includes('user')) {
      return <UserIcon size={16} className="text-white" />;
  ...
  const getActivityBackgroundColor = (action: string) => {
    // Apply same colors as AdminUsers
    if (action.toLowerCase().includes('user')) {
                    ...
                    <Card key={log.id} className="border-gray-200 hover:shadow-md transition-shadow">
                      <CardContent className="p-4">
                        <div className="flex items-start gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${getActivityBackgroundColor(log.action)}`}>
